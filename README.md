...
```mermaid
flowchart LR
  UI[React Renderer] -->|受限 Workspace API| Preload[Context-isolated Preload]
  Preload -->|白名单 IPC| Main[Electron Main Process]
  Main --> FS[本地需求目录]
...
Sidecar 启动时会自动选择可用的本地端口。若 Sidecar 不可用，桌面窗口仍可打开，侧栏底部会显示异常状态。
...
│   ├── features/workbench/         # 应用级停靠面板和标签状态
│   ├── pages/                      # 新对话、定时任务、需求详情
│   ├── App.tsx                     # 应用壳、侧栏和路由
│   └── styles.css                  # 全局界面样式
├── scripts/
...
- 空间、需求和定时任务主要保存在前端运行状态中，重启应用后会恢复默认数据。
